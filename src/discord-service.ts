...
let discordWriteQueueDepth = 0;
let discordWriteOperationCount = 0;
let lastDiscordWriteError: string | null = null;
let lastDiscordWriteFinishedAt: string | null = null;
...
async function enqueueDiscordWrite<T>(operation: () => Promise<T>): Promise<T> {
  discordWriteQueueDepth += 1;

  const queued = discordWriteQueueTail.then(
    async () => {
      discordWriteOperationCount += 1;

      try {
        return await operation();
      } catch (error) {
        lastDiscordWriteError = error instanceof Error ? error.message : "Unknown Discord write error";
        throw error;
      } finally {
        discordWriteQueueDepth = Math.max(0, discordWriteQueueDepth - 1);
        lastDiscordWriteFinishedAt = new Date().toISOString();
      }
    },
    async () => {
      discordWriteOperationCount += 1;
...
export async function getDiscordBotDiagnostics(): Promise<{
  ok: boolean;
  checkedAt: string;
  service: "discasa_bot";
  runtime: Awaited<ReturnType<typeof getDiscordBotRuntimeStatus>>;
  queue: {
  ...
  const runtime = await getDiscordBotRuntimeStatus();

  return {
    ok: runtime.mockMode || (runtime.botConfigured && runtime.botLoggedIn),
    checkedAt: new Date().toISOString(),
    service: "discasa_bot",
    runtime,
    queue: {
      pendingOrRunningWrites: discordWriteQueueDepth,
      completedOrStartedWrites: discordWriteOperationCount,
      lastError: lastDiscordWriteError,
      lastFinishedAt: lastDiscordWriteFinishedAt,
    },
    storage: {
      uploadLimitBytes: DISCASA_UPLOAD_LIMIT_BYTES,
      uploadLimitLabel: formatBytes(DISCASA_UPLOAD_LIMIT_BYTES),
    },
  };
}
...
async function readJsonSnapshot(channelId: string, fileNames: string[]): Promise<{ payload: unknown; fileName: string } | null> {
  ...
  try {
    ...
    logger.warn(`[Discasa snapshot] Failed to parse ${found.fileName} from channel ${channelId}.`);
    return null;
  }
}
